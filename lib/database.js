...
export async function createUrun(urunData) {
  try {
    // Seçilen kategorideki en yüksek sıra numarasını al
    const { data: maxSiraData } = await supabase
      .from("urunler")
      .select("sira")
      .eq("kategori_id", urunData.kategori_id)
      .order("sira", { ascending: false })
      .limit(1)
      .single();
    ...
    // Yeni ürün verisi
    const yeniUrun = {
      baslik: urunData.baslik,
      aciklama: urunData.aciklama,
      fiyat: urunData.fiyat,
      resim: urunData.resim,
      kategori_id: urunData.kategori_id,
      sira: yeniSira,
    };
    ...
    console.log("Yeni ürün oluşturuldu:", data);
    return { data, error: null };
  } catch (error) {
    console.error("Ürün oluşturulurken hata:", error);
    return { data: null, error };
...
export async function updateUrun(urunId, urunData) {
  try {
    
    console.log("Ürün güncelleme verisi sira için:", urunData);
    ...
    console.error("updateUrun Hata:", error);
    return { data: null, error };
  }
}

export async function updateUrunSira(id, yeniSira) {
  ...
  if (error) {
    console.error("Ürün sırası güncellenirken hata:", error);
    return { data: null, error };
  }
...
export async function removeUrun(id) {
  try {
